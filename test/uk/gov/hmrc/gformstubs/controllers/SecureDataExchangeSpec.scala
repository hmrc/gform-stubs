...
class SecureDataExchangeSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {
  val fakeRequest = FakeRequest("GET", "/")

  "SecureDataExchange" should {
    "return 200 with the public key" in {
      val controller = new SecureDataExchange(stubControllerComponents())
      val result = controller.getPublicKey()(fakeRequest)
